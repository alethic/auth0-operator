...
namespace Alethic.Auth0.Operator.Controllers
{
        ...
        IEntityController<V2alpha3Client>
    {
        ...
        [return: NotNullIfNotNull(nameof(source))]
        internal static V2alpha3ClientRefreshTokenPolicy? FromApi(ClientRefreshTokenPolicy? source) => source is null ? null : new()
        {
        ...
        /// <summary>
        /// Determines whether the client differs from the desired configuration, comparing exactly the properties
        /// transmitted by <see cref="ApplyToApi(V2alpha3ClientConf, UpdateClientRequestContent)"/>. Properties unset
        /// on <paramref name="conf"/> are unmanaged by the spec and ignored. A key present in the last-known client
        /// metadata but absent from the spec metadata still requires an update, because the update request tombstones
        /// such keys to delete them. Excluded from comparison: <c>SigningKeys</c> (read-only, generated by Auth0) and
        /// <c>ResourceServers</c> (never transmitted to Auth0).
        /// </summary>
        internal static bool ConfRequiresUpdate(V2alpha3ClientConf conf, V2alpha3ClientConf last)
        {
            ...
            if (conf.ApplicationType is not null && conf.ApplicationType != last.ApplicationType)
                return true;

            if (conf.TokenEndpointAuthMethod is not null && conf.TokenEndpointAuthMethod != last.TokenEndpointAuthMethod)
                return true;

            if (RequiresUpdate(conf.AddOns, last.AddOns))
                return true;

            if (conf.AllowedClients is not null && (last.AllowedClients is null || conf.AllowedClients.ToHashSet().SetEquals(last.AllowedClients) == false))
                return true;
            ...
            if (conf.AllowedOrigins is not null && (last.AllowedOrigins is null || conf.AllowedOrigins.ToHashSet().SetEquals(last.AllowedOrigins) == false))
                return true;

            if (conf.WebOrigins is not null && (last.WebOrigins is null || conf.WebOrigins.ToHashSet().SetEquals(last.WebOrigins) == false))
                return true;

            if (conf.InitiateLoginUri is not null && conf.InitiateLoginUri != last.InitiateLoginUri)
                return true;

            if (conf.Callbacks is not null && (last.Callbacks is null || conf.Callbacks.ToHashSet().SetEquals(last.Callbacks) == false))
                return true;
            ...
            if (conf.ClientAliases is not null && (last.ClientAliases is null || conf.ClientAliases.ToHashSet().SetEquals(last.ClientAliases) == false))
                return true;

            if (RequiresUpdate(conf.ClientMetaData, last.ClientMetaData))
                return true;

            if (conf.IsCustomLoginPageOn is not null && conf.IsCustomLoginPageOn != last.IsCustomLoginPageOn)
                return true;

            if (conf.IsFirstParty is not null && conf.IsFirstParty != last.IsFirstParty)
                return true;

            if (conf.CustomLoginPage is not null && conf.CustomLoginPage != last.CustomLoginPage)
                return true;

            if (conf.CustomLoginPagePreview is not null && conf.CustomLoginPagePreview != last.CustomLoginPagePreview)
                return true;

            if (RequiresUpdate(conf.EncryptionKey, last.EncryptionKey))
                return true;

            if (conf.FormTemplate is not null && conf.FormTemplate != last.FormTemplate)
                return true;

            if (conf.GrantTypes is not null && (last.GrantTypes is null || conf.GrantTypes.ToHashSet().SetEquals(last.GrantTypes) == false))
                return true;

            if (RequiresUpdate(conf.JwtConfiguration, last.JwtConfiguration))
                return true;

            if (RequiresUpdate(conf.Mobile, last.Mobile))
                return true;
            ...
            if (conf.LogoUri is not null && conf.LogoUri != last.LogoUri)
                return true;

            if (conf.OidcConformant is not null && conf.OidcConformant != last.OidcConformant)
                return true;

            if (RequiresUpdate(conf.OidcLogout, last.OidcLogout))
                return true;

            if (conf.Sso is not null && conf.Sso != last.Sso)
                return true;

            if (RequiresUpdate(conf.RefreshToken, last.RefreshToken))
                return true;

            if (conf.OrganizationUsage is not null && conf.OrganizationUsage != last.OrganizationUsage)
                return true;

            if (conf.OrganizationRequireBehavior is not null && conf.OrganizationRequireBehavior != last.OrganizationRequireBehavior)
                return true;

            if (conf.CrossOriginAuthentication is not null && conf.CrossOriginAuthentication != last.CrossOriginAuthentication)
                return true;

            if (conf.RequirePushedAuthorizationRequests is not null && conf.RequirePushedAuthorizationRequests != last.RequirePushedAuthorizationRequests)
                return true;

            if (RequiresUpdate(conf.DefaultOrganization, last.DefaultOrganization))
                return true;

            if (conf.ComplianceLevel is not null && conf.ComplianceLevel != last.ComplianceLevel)
                return true;
        ...
        /// <summary>
        /// Returns <c>true</c> if any key of <paramref name="conf"/> is missing from <paramref name="last"/> or holds
        /// a different value. Keys present only on <paramref name="last"/> are ignored here; removal is handled by the
        /// tombstone rule in <see cref="ConfRequiresUpdate(V2alpha3ClientConf, V2alpha3ClientConf)"/> for the
        /// dictionaries where the update request supports deletion.
        /// </summary>
        static bool RequiresUpdate(Dictionary<string, object?>? conf, Dictionary<string, object?>? last)
        {
            ...
            foreach (var (key, value) in conf)
            {
                if (last.TryGetValue(key, out var lastValue) == false)
                    return true;

                if (Equals(value?.ToString(), lastValue?.ToString()) == false)
                    return true;
            }
        ...
        static bool RequiresUpdate(V2alpha3ClientEncryptionKey? conf, V2alpha3ClientEncryptionKey? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.Pub is not null && conf.Pub != last.Pub)
                return true;
        ...
        static bool RequiresUpdate(V2alpha3ClientJwtConfiguration? conf, V2alpha3ClientJwtConfiguration? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.LifetimeInSeconds is not null && conf.LifetimeInSeconds != last.LifetimeInSeconds)
                return true;

            if (conf.SecretEncoded is not null && conf.SecretEncoded != last.SecretEncoded)
                return true;

            if (RequiresUpdate(conf.Scopes, last.Scopes))
                return true;

            if (conf.Alg is not null && conf.Alg != last.Alg)
                return true;

            return false;
        }

        static bool RequiresUpdate(V2alpha3ClientMobile? conf, V2alpha3ClientMobile? last)
        {
            if (conf is null)
                return false;
            ...
            if (RequiresUpdate(conf.Android, last.Android))
                return true;

            if (RequiresUpdate(conf.Ios, last.Ios))
                return true;

            return false;
        }

        static bool RequiresUpdate(V2alpha3ClientMobileAndroid? conf, V2alpha3ClientMobileAndroid? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.AppPackageName is not null && conf.AppPackageName != last.AppPackageName)
                return true;

            if (conf.Sha256CertFingerprints is not null && (last.Sha256CertFingerprints is null || conf.Sha256CertFingerprints.ToHashSet().SetEquals(last.Sha256CertFingerprints) == false))
                return true;

            return false;
        }

        static bool RequiresUpdate(V2alpha3ClientMobileiOs? conf, V2alpha3ClientMobileiOs? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.AppBundleIdentifier is not null && conf.AppBundleIdentifier != last.AppBundleIdentifier)
                return true;

            return false;
        }

        static bool RequiresUpdate(V2alpha3ClientOidcBackchannelLogoutSettings? conf, V2alpha3ClientOidcBackchannelLogoutSettings? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.BackchannelLogoutUrls is not null && (last.BackchannelLogoutUrls is null || conf.BackchannelLogoutUrls.ToHashSet().SetEquals(last.BackchannelLogoutUrls) == false))
                return true;

            if (RequiresUpdate(conf.BackchannelLogoutInitiators, last.BackchannelLogoutInitiators))
                return true;

            if (RequiresUpdate(conf.BackchannelLogoutSessionMetadata, last.BackchannelLogoutSessionMetadata))
                return true;

            return false;
        }

        static bool RequiresUpdate(V2alpha3ClientOidcBackchannelLogoutInitiators? conf, V2alpha3ClientOidcBackchannelLogoutInitiators? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.SelectedInitiators is not null && (last.SelectedInitiators is null || conf.SelectedInitiators.ToHashSet().SetEquals(last.SelectedInitiators) == false))
                return true;

            return false;
        }

        static bool RequiresUpdate(V2alpha3ClientOidcBackchannelLogoutSessionMetadata? conf, V2alpha3ClientOidcBackchannelLogoutSessionMetadata? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.Include is not null && conf.Include != last.Include)
                return true;

            return false;
        }

        static bool RequiresUpdate(V2alpha3ClientRefreshTokenConfiguration? conf, V2alpha3ClientRefreshTokenConfiguration? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.RotationType is not null && conf.RotationType != last.RotationType)
                return true;

            if (conf.ExpirationType is not null && conf.ExpirationType != last.ExpirationType)
                return true;

            if (conf.Leeway is not null && conf.Leeway != last.Leeway)
                return true;

            if (conf.TokenLifetime is not null && conf.TokenLifetime != last.TokenLifetime)
                return true;

            if (conf.InfiniteTokenLifetime is not null && conf.InfiniteTokenLifetime != last.InfiniteTokenLifetime)
                return true;

            if (conf.IdleTokenLifetime is not null && conf.IdleTokenLifetime != last.IdleTokenLifetime)
                return true;

            if (conf.InfiniteIdleTokenLifetime is not null && conf.InfiniteIdleTokenLifetime != last.InfiniteIdleTokenLifetime)
                return true;

            if (RequiresUpdate(conf.Policies, last.Policies))
                return true;
        ...
        /// <summary>
        /// Compares refresh token policies as a set keyed by audience — ordering carries no meaning — with each
        /// policy's scope compared as an unordered set.
        /// </summary>
        static bool RequiresUpdate(V2alpha3ClientRefreshTokenPolicy[]? conf, V2alpha3ClientRefreshTokenPolicy[]? last)
        {
            if (conf is null)
                return false;

            if (last is null || conf.Length != last.Length)
                return true;

            foreach (var policy in conf)
            {
                var match = last.FirstOrDefault(i => i.Audience == policy.Audience);
                if (match is null)
                    return true;

                if (policy.Scope is not null && (match.Scope is null || policy.Scope.ToHashSet().SetEquals(match.Scope) == false))
                    return true;
            }
        ...
        static bool RequiresUpdate(V2alpha3ClientDefaultOrganization? conf, V2alpha3ClientDefaultOrganization? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.OrganizationId is not null && conf.OrganizationId != last.OrganizationId)
                return true;

            if (conf.Flows is not null && (last.Flows is null || conf.Flows.ToHashSet().SetEquals(last.Flows) == false))
                return true;

            return false;
        }

        static bool RequiresUpdate(V2alpha3ClientAddons? conf, V2alpha3ClientAddons? last)
        {
            if (conf is null)
                return false;
            ...
            if (RequiresUpdate(conf.Aws, last.Aws))
                return true;

            if (RequiresUpdate(conf.AzureBlob, last.AzureBlob))
                return true;

            if (RequiresUpdate(conf.AzureSb, last.AzureSb))
                return true;

            if (RequiresUpdate(conf.Rms, last.Rms))
                return true;

            if (RequiresUpdate(conf.Mscrm, last.Mscrm))
                return true;

            if (RequiresUpdate(conf.Slack, last.Slack))
                return true;

            if (RequiresUpdate(conf.Sentry, last.Sentry))
                return true;

            if (RequiresUpdate(conf.Box, last.Box))
                return true;

            if (RequiresUpdate(conf.Cloudbees, last.Cloudbees))
                return true;

            if (RequiresUpdate(conf.Concur, last.Concur))
                return true;

            if (RequiresUpdate(conf.Dropbox, last.Dropbox))
                return true;

            if (RequiresUpdate(conf.Echosign, last.Echosign))
                return true;

            if (RequiresUpdate(conf.Egnyte, last.Egnyte))
                return true;

            if (RequiresUpdate(conf.Firebase, last.Firebase))
                return true;

            if (RequiresUpdate(conf.Newrelic, last.Newrelic))
                return true;
            ...
            if (RequiresUpdate(conf.SalesforceApi, last.SalesforceApi))
                return true;

            if (RequiresUpdate(conf.SalesforceSandboxApi, last.SalesforceSandboxApi))
                return true;

            if (RequiresUpdate(conf.Samlp, last.Samlp))
                return true;

            if (RequiresUpdate(conf.Layer, last.Layer))
                return true;

            if (RequiresUpdate(conf.SapApi, last.SapApi))
                return true;

            if (RequiresUpdate(conf.Sharepoint, last.Sharepoint))
                return true;

            if (RequiresUpdate(conf.Springcm, last.Springcm))
                return true;

            if (RequiresUpdate(conf.Wams, last.Wams))
                return true;

            if (RequiresUpdate(conf.Wsfed, last.Wsfed))
                return true;

            if (RequiresUpdate(conf.Zendesk, last.Zendesk))
                return true;

            if (RequiresUpdate(conf.Zoom, last.Zoom))
                return true;

            if (RequiresUpdate(conf.SsoIntegration, last.SsoIntegration))
                return true;

            if (RequiresUpdate(conf.Oag, last.Oag))
                return true;

            return false;
        }

        static bool RequiresUpdate(V2alpha3ClientAddonAws? conf, V2alpha3ClientAddonAws? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.Principal is not null && conf.Principal != last.Principal)
                return true;

            if (conf.Role is not null && conf.Role != last.Role)
                return true;
        ...
        static bool RequiresUpdate(V2alpha3ClientAddonAzureBlob? conf, V2alpha3ClientAddonAzureBlob? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.AccountName is not null && conf.AccountName != last.AccountName)
                return true;

            if (conf.StorageAccessKey is not null && conf.StorageAccessKey != last.StorageAccessKey)
                return true;

            if (conf.ContainerName is not null && conf.ContainerName != last.ContainerName)
                return true;

            if (conf.BlobName is not null && conf.BlobName != last.BlobName)
                return true;

            if (conf.Expiration is not null && conf.Expiration != last.Expiration)
                return true;

            if (conf.SignedIdentifier is not null && conf.SignedIdentifier != last.SignedIdentifier)
                return true;

            if (conf.BlobRead is not null && conf.BlobRead != last.BlobRead)
                return true;

            if (conf.BlobWrite is not null && conf.BlobWrite != last.BlobWrite)
                return true;

            if (conf.BlobDelete is not null && conf.BlobDelete != last.BlobDelete)
                return true;

            if (conf.ContainerRead is not null && conf.ContainerRead != last.ContainerRead)
                return true;

            if (conf.ContainerWrite is not null && conf.ContainerWrite != last.ContainerWrite)
                return true;

            if (conf.ContainerDelete is not null && conf.ContainerDelete != last.ContainerDelete)
                return true;

            if (conf.ContainerList is not null && conf.ContainerList != last.ContainerList)
                return true;

            return false;
        }

        static bool RequiresUpdate(V2alpha3ClientAddonAzureSb? conf, V2alpha3ClientAddonAzureSb? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.Namespace is not null && conf.Namespace != last.Namespace)
                return true;

            if (conf.SasKeyName is not null && conf.SasKeyName != last.SasKeyName)
                return true;

            if (conf.SasKey is not null && conf.SasKey != last.SasKey)
                return true;

            if (conf.EntityPath is not null && conf.EntityPath != last.EntityPath)
                return true;
        ...
        static bool RequiresUpdate(V2alpha3ClientAddonRms? conf, V2alpha3ClientAddonRms? last)
        {
            if (conf is null)
                return false;
        ...
        static bool RequiresUpdate(V2alpha3ClientAddonMscrm? conf, V2alpha3ClientAddonMscrm? last)
        {
            if (conf is null)
                return false;
        ...
        static bool RequiresUpdate(V2alpha3ClientAddonSlack? conf, V2alpha3ClientAddonSlack? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.Team is not null && conf.Team != last.Team)
                return true;

            return false;
        }

        static bool RequiresUpdate(V2alpha3ClientAddonSentry? conf, V2alpha3ClientAddonSentry? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.OrgSlug is not null && conf.OrgSlug != last.OrgSlug)
                return true;
        ...
        static bool RequiresUpdate(V2alpha3ClientAddonEchoSign? conf, V2alpha3ClientAddonEchoSign? last)
        {
            if (conf is null)
                return false;
        ...
        static bool RequiresUpdate(V2alpha3ClientAddonEgnyte? conf, V2alpha3ClientAddonEgnyte? last)
        {
            if (conf is null)
                return false;
        ...
        static bool RequiresUpdate(V2alpha3ClientAddonFirebase? conf, V2alpha3ClientAddonFirebase? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.PrivateKeyId is not null && conf.PrivateKeyId != last.PrivateKeyId)
                return true;

            if (conf.PrivateKey is not null && conf.PrivateKey != last.PrivateKey)
                return true;

            if (conf.ClientEmail is not null && conf.ClientEmail != last.ClientEmail)
                return true;
        ...
        static bool RequiresUpdate(V2alpha3ClientAddonNewRelic? conf, V2alpha3ClientAddonNewRelic? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.Account is not null && conf.Account != last.Account)
                return true;

            return false;
        }

        static bool RequiresUpdate(V2alpha3ClientAddonOffice365? conf, V2alpha3ClientAddonOffice365? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.Connection is not null && conf.Connection != last.Connection)
                return true;

            return false;
        }

        static bool RequiresUpdate(V2alpha3ClientAddonSalesforce? conf, V2alpha3ClientAddonSalesforce? last)
        {
            if (conf is null)
                return false;
        ...
        static bool RequiresUpdate(V2alpha3ClientAddonSalesforceApi? conf, V2alpha3ClientAddonSalesforceApi? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.CommunityName is not null && conf.CommunityName != last.CommunityName)
                return true;

            if (conf.CommunityUrlSection is not null && conf.CommunityUrlSection != last.CommunityUrlSection)
                return true;

            return false;
        }

        static bool RequiresUpdate(V2alpha3ClientAddonSalesforceSandboxApi? conf, V2alpha3ClientAddonSalesforceSandboxApi? last)
        {
            if (conf is null)
                return false;
        ...
        static bool RequiresUpdate(V2alpha3ClientAddonSaml? conf, V2alpha3ClientAddonSaml? last)
        {
            if (conf is null)
                return false;
            ...
            if (RequiresUpdate(conf.Mappings, last.Mappings))
                return true;

            if (conf.Audience is not null && conf.Audience != last.Audience)
                return true;

            if (conf.Recipient is not null && conf.Recipient != last.Recipient)
                return true;

            if (conf.CreateUpnClaim is not null && conf.CreateUpnClaim != last.CreateUpnClaim)
                return true;

            if (conf.MapUnknownClaimsAsIs is not null && conf.MapUnknownClaimsAsIs != last.MapUnknownClaimsAsIs)
                return true;

            if (conf.PassthroughClaimsWithNoMapping is not null && conf.PassthroughClaimsWithNoMapping != last.PassthroughClaimsWithNoMapping)
                return true;

            if (conf.MapIdentities is not null && conf.MapIdentities != last.MapIdentities)
                return true;
            ...
            if (conf.Destination is not null && conf.Destination != last.Destination)
                return true;

            if (conf.LifetimeInSeconds is not null && conf.LifetimeInSeconds != last.LifetimeInSeconds)
                return true;

            if (conf.SignResponse is not null && conf.SignResponse != last.SignResponse)
                return true;

            if (conf.NameIdentifierFormat is not null && conf.NameIdentifierFormat != last.NameIdentifierFormat)
                return true;

            // name identifier probes are tried in order, so ordering is semantic and compared as-is
            if (conf.NameIdentifierProbes is not null && (last.NameIdentifierProbes is null || conf.NameIdentifierProbes.SequenceEqual(last.NameIdentifierProbes) == false))
                return true;

            if (conf.AuthnContextClassRef is not null && conf.AuthnContextClassRef != last.AuthnContextClassRef)
                return true;

            return false;
        }

        static bool RequiresUpdate(V2alpha3ClientAddonLayer? conf, V2alpha3ClientAddonLayer? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.ProviderId is not null && conf.ProviderId != last.ProviderId)
                return true;

            if (conf.KeyId is not null && conf.KeyId != last.KeyId)
                return true;
        ...
        static bool RequiresUpdate(V2alpha3ClientAddonSapapi? conf, V2alpha3ClientAddonSapapi? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.UsernameAttribute is not null && conf.UsernameAttribute != last.UsernameAttribute)
                return true;

            if (conf.TokenEndpointUrl is not null && conf.TokenEndpointUrl != last.TokenEndpointUrl)
                return true;

            if (conf.Scope is not null && conf.Scope != last.Scope)
                return true;

            if (conf.ServicePassword is not null && conf.ServicePassword != last.ServicePassword)
                return true;
        ...
        static bool RequiresUpdate(V2alpha3ClientAddonSharePoint? conf, V2alpha3ClientAddonSharePoint? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.ExternalUrl is not null && (last.ExternalUrl is null || conf.ExternalUrl.ToHashSet().SetEquals(last.ExternalUrl) == false))
                return true;

            return false;
        }

        static bool RequiresUpdate(V2alpha3ClientAddonSpringCm? conf, V2alpha3ClientAddonSpringCm? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.Acsurl is not null && conf.Acsurl != last.Acsurl)
                return true;

            return false;
        }

        static bool RequiresUpdate(V2alpha3ClientAddonWams? conf, V2alpha3ClientAddonWams? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.Masterkey is not null && conf.Masterkey != last.Masterkey)
                return true;

            return false;
        }

        static bool RequiresUpdate(V2alpha3ClientAddonZendesk? conf, V2alpha3ClientAddonZendesk? last)
        {
            if (conf is null)
                return false;
        ...
        static bool RequiresUpdate(V2alpha3ClientAddonZoom? conf, V2alpha3ClientAddonZoom? last)
        {
            if (conf is null)
                return false;
        ...
        static bool RequiresUpdate(V2alpha3ClientAddonSsoIntegration? conf, V2alpha3ClientAddonSsoIntegration? last)
        {
            if (conf is null)
                return false;
            ...
            if (conf.Version is not null && conf.Version != last.Version)
                return true;

            return false;
        }

        static bool RequiresUpdate(V2alpha3ClientAddonOag? conf, V2alpha3ClientAddonOag? last)
        {
            // the OAG addon has no properties; only its presence can differ
            if (conf is null)
                return false;
