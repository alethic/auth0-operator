...
namespace Alethic.Auth0.Operator.Controllers
{
        ...
        where TLastConf : class
    {
        ...
        async Task<ReconciliationResult<TEntity>> IEntityController<TEntity>.DeletedAsync(TEntity entity, CancellationToken cancellationToken)
        {
            ...
            {
                Logger.LogError(e, "Retry exception hit deleting {EntityTypeName} {EntityNamespace}/{EntityName}: {Message}", EntityTypeName, entity.Namespace(), entity.Name(), e.Message);
            ...
            {
                Logger.LogError(e, "Unhandled exception hit deleting {EntityTypeName} {EntityNamespace}/{EntityName}: {Message}", EntityTypeName, entity.Namespace(), entity.Name(), e.Message);
